...
<section class="meetings-page" id="meetings">
    ...
    <!--  message box -->
        ...
        <!-- 	  Sidebar Dashboard -->
  		 <div class="col-lg-3 my-lg-0 my-md-1">
            <div id="sidebar" class="bg-purple">
                <div class="h4 text-white">Student Information</div>
                <ul>    
      ...
      <div class="row">
        ...
        <h4 style="color: aliceblue;" class="text-right"></h4>
         <security:authorize access="hasRole('ADMIN')"> 
         <div><a  class="btn btn-primary" href="/addstudent">Add Student</a></div> 
         </security:authorize> 
    </div>
        ...
        <table class="table bg-light ">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>${student.studentPrnNo}</td>
                 <security:authorize access="hasRole('ADMIN')"> 
                <td><a class="btn btn-primary" href="/updatestudent?id=${student.studentId}">Edit</a></td>  
   				<td><a class="btn btn-danger" href="/deletestudent?id=${student.studentId}">Delete</a></td>  
   				  </security:authorize> 
              </tr>
            </c:forEach>             
            </tbody>
    ...
    <c:if test="${pageNumber gt 1}">
        <a href="studentpage?pageNumber=${pageNumber - 1}">Previous</a>
    </c:if>
    <c:forEach begin="1" end="${numberOfPages}" var="i">
        <c:choose>
            <c:when test="${i!=pageNumber}">
           		 <a  href="studentpage?pageNumber=<c:out value="${i}"/>"><c:out value="${i}"/></a>
            </c:when>
        	<c:otherwise>
        		<c:out value="${i}"/>
        	</c:otherwise>        
    ...
    <c:if test="${pageNumber lt numberOfPages}">
        <a href="studentpage?pageNumber=${pageNumber + 1}">Next</a>
    </c:if>
         </div>
       <%--End of pagination --%>
       </div>
