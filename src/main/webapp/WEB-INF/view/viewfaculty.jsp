...
<section class="meetings-page" id="meetings">
 <!--  message box -->
    ...
    <!-- 	  Sidebar Dashboard -->
  		 <div class="col-lg-3 my-lg-0 my-md-1">
            <div id="sidebar" class="bg-purple">
                <div class="h4 text-white">Faculty Info</div>
                <ul>    
                 <li> <a href="/viewallschedule" class="text-decoration-none d-flex align-items-start">
                    ...
                    <li> <a href="/listmeeting" class="text-decoration-none d-flex align-items-start">
                      ...
                      <li class="active" > <a href="/viewallfaculty" class="text-decoration-none d-flex align-items-start">
      ...
      <div class="row">
        ...
        <security:authorize access="hasRole('ADMIN')">  
        <div><a  class="btn btn-primary" href="/addfaculty">Add Faculty</a></div> 
        </security:authorize> 
       ...
       <table class="table bg-light ">
            ...
            <tbody>
                <c:forEach items="${faculty}" var="faculty" varStatus="counter" >
              <tr>
                <th scope="row">${counter.count}</th>
                <td>${faculty.facultyFname } ${faculty.facultyLname}</td>
                <td>${faculty.facultyMobileNo}</td>
                <td>${faculty.facultyEmail}</td>
                <td>${faculty.facultyExperienceInYears}</td>
                <security:authorize access="hasRole('ADMIN')">  
                <td><a class="btn btn-primary" href="/updatefaculty?id=${faculty.facultyId}">Edit</a></td>  
   				<td><a class="btn btn-danger" href="/deletefaculty?id=${faculty.facultyId}">Delete</a></td>  
   				</security:authorize> 
              </tr>
            </c:forEach>             
            </tbody>
    ...
    <c:if test="${pageNumber gt 1}">
        <a href="facultypage?pageNumber=${pageNumber - 1}">Previous</a>
    </c:if>
    <c:forEach begin="1" end="${numberOfPages}" var="i">
        <c:choose>
            <c:when test="${i!=pageNumber}">
           		 <a  href="facultypage?pageNumber=<c:out value="${i}"/>"><c:out value="${i}"/></a>
            </c:when>
        	<c:otherwise>
        		<c:out value="${i}"/>
        	</c:otherwise>        
    ...
    <c:if test="${pageNumber lt numberOfPages}">
        <a href="facultypage?pageNumber=${pageNumber + 1}">Next</a>
    </c:if>
         </div>
       <%--End of pagination --%>
          </div>
